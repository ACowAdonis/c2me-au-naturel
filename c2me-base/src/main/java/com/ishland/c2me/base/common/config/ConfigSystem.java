...
public class ConfigSystem {
    ...
    public static class ConfigAccessor {
        ...
        public java.util.List<String> getStringList(java.util.List<String> def, java.util.List<String> incompatibleDef) {
            findModDefinedIncompatibility();
            final Object configured = CONFIG.get(this.key);
            boolean isDefaultValue;
            if (configured != null) {
                if (configured instanceof java.util.List) {
                    isDefaultValue = false;
                } else {
                    LOGGER.warn("Invalid configured value: {} -> {}", this.key, configured);
                    CONFIG.remove(this.key);
            ...
            generateListDefaultEntry(def, incompatibleDef);

            if (this.incompatibilityDetected) return incompatibleDef;
            if (isDefaultValue) return def;
            java.util.List<?> rawList = CONFIG.get(this.key);
            return rawList.stream().map(String::valueOf).collect(java.util.stream.Collectors.toList());
        }

        private void generateListDefaultEntry(java.util.List<String> def, java.util.List<String> incompatibleDef) {
            if (!CONFIG.contains(this.key)) {
                CONFIG.set(this.key, def);
            }
