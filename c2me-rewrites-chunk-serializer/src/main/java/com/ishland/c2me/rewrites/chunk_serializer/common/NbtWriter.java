...
public class NbtWriter {
    ...
    public static byte @NotNull [] getStringBytes(String string) {
        // C2ME fix: Avoid char array allocation by using charAt(), and optimize final array copy
        final int stringLength = string.length();
        byte[] res = new byte[stringLength * 3 + 2];
        int index = 2;
        ...
        // C2ME fix: If the result fits in the initial allocation, reuse it
        if (index == res.length) {
            return res;
        }
