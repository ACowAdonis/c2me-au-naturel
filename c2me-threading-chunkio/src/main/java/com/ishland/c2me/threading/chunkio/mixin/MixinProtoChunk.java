...
public class MixinProtoChunk implements ProtoChunkExtension {
    ...
    public boolean getNeedBlending() {
        // C2ME fix: Don't block with join() - this can cause thread pool exhaustion deadlock
        // If all worker threads block on join(), no threads are left to complete the futures,
        // causing a deadlock during world creation/loading.
        // Instead, return true (safe default - will blend if needed) when not yet computed.
        if (!blendingComputeFuture.isDone()) {
            return true; // Safe default: assume blending needed until proven otherwise
        }
        return needBlending;
    }
