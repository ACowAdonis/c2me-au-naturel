...
public class PooledFeatureContext<FC extends FeatureConfig> extends FeatureContext<FC> {
    ...
    // C2ME fix: Reduced from 2048 to 256 to save memory (reduces per-thread overhead from ~200KB to ~25KB)
    // Most worldgen operations don't need this many pooled contexts simultaneously
    public static final ThreadLocal<SimpleObjectPool<PooledFeatureContext<?>>> POOL = ThreadLocal.withInitial(() -> new SimpleObjectPool<>(unused -> new PooledFeatureContext<>(), unused -> {}, 256));
